...
Subroutine  GET_geo_sngl_geo(NCID,mscNlon,mscNlat,mscValueLAT,mscValueLON, &
            mscValueLand,mscValueLanduse,mscValueLandusefr)
!
...
!
  ...
  INTEGER ::  NCID, STATUS, MSLATID,MSLONID,MSLANDID,MSLUSEFRID
  ...
  REAL ::   mscValueLandusefr(mscNlon,mscNlat,21)
  INTEGER :: i,j
...
!  COUNT(3)=21
!  STATUS = NF_INQ_VARID (NCID, 'LANDUSEF', MSLUSEFRID)
!  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_geo(STATUS)
!  STATUS = NF_GET_VARA_REAL (NCID, MSLUSEFRID, START, COUNT, mscValueLandusefr)
!  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_geo(STATUS)
