...
!
  ...
  numvar_sfc=0
  varlist_sfc="2mt"
  tracerfile="fv_tracer.res.tile1.nc"
  numvar_tracer=13
  ...
  filelist(1)=trim(dynfile)
  filelist(2)=trim(tracerfile)
  filelist(3)=trim(sfcfile)
...
! now read in each fields from fv3 file
     ...
     do iens=1,num_ensemble
        iret=nf90_open(filelist(mype_fileid),nf90_nowrite,ncioid,comm=mpi_comm_world,info=MPI_INFO_NULL)
        if(iret/=nf90_noerr) then
           write(6,*)' problem opening', trim(filelist(mype_fileid)),', Status =',iret
           call flush(6)
           stop(333)
        endif
...
!
     ...
     do ilev=mype_lbegin,mype_lend
        write(6,'(I5,A10,I5,2f15.7)') mype_fileid,trim(adjustl(mype_varname)),ilev,  &
