subroutine pullandcalc(nx,ny,fnuma,bnum,dname)
  ...
  real(4),allocatable::maxrh(:,:),tempfld(:,:),dewfld(:,:)
  real(4)::intemp,indew,outrh,anlmax,bgmax
  ...
  if (bnum.eq.1) then
     do i=1,nx
        do j=1,ny
           maxrh(i,j)=maxval(rhbg(i,j,:))
           if (j.eq.100.and.i.eq.100) then
              print*, "Max RH value at 100,100=",maxrh(100,100)
           endif
        enddo
     enddo
     write(fnuma) maxrh
  ...
  elseif (bnum.eq.2) then
     do i=1,nx
        do j=1,ny
           maxrh(i,j)=maxval(rhanl(i,j,:))
           if (j.eq.100.and.i.eq.100) then
              print*, "Max RH value at 100,100=",maxrh(100,100)
           endif
!           print*,'maxrh,i,j=',maxrh(i,j),i,j
        enddo
     enddo
     write(fnuma) maxrh
     deallocate(tempanl)
  ...
  elseif (bnum.eq.3) then
     do i=1,nx
        do j=1,ny
           bgmax=maxval(rhbg(i,j,:))
           anlmax=maxval(rhanl(i,j,:))
           if (bgmax.ge.anlmax) then
              maxrh(i,j)=bgmax
           else
              maxrh(i,j)=anlmax
           endif
